...
export function ChooseBarber() {
  ...
  const selectedBarber = useBookingStore((state) => state.barberId);
  const setSelectedBarber = useBookingStore((state) => state.setBarber);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {isLoading ? <p>Loading barbers...</p> : barbers.map((barber: any) => (
          <div
            key={barber.id}
            onClick={() => barber.status === 'Available' && setSelectedBarber(barber.id)}
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">{barber.experience_years} yrs exp</p>
