...
import { fetchBarberQueue } from "../../api/client";

export function QueueStatus() {
  const navigate = useNavigate();
  
  const MOCK_BARBER_ID = "00000000-0000-0000-0000-000000000000";
  const { data: queue = [] } = useQuery({ queryKey: ["queue"], queryFn: () => fetchBarberQueue(MOCK_BARBER_ID) });
  
  const customersAhead = queue.length;
      ...
      <div className="bg-card border border-border rounded-3xl p-8 text-center shadow-sm relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 text-left">
          ...
          <div className="bg-secondary/50 p-4 rounded-2xl border border-border">
            ...
            <p className="text-2xl font-bold">{customersAhead}</p>
          </div>
        </div>
