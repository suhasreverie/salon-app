...
export function SelectDateTime() {
  ...
  // When date changes, update store but clear time selection
  useEffect(() => {
    setDateTime(format(selectedDate, 'yyyy-MM-dd'), "");
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {times.map((time) => {
              ...
              return (
                <button
                  key={time}
                  onClick={() => !isUnavailable && setDateTime(format(selectedDate, 'yyyy-MM-dd'), time)}
