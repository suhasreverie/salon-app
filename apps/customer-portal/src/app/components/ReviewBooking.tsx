...
export function ReviewBooking() {
  ...
  // HARDCODED CUSTOMER ID FOR PROOF OF CONCEPT
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="bg-card border border-border rounded-3xl p-6 md:p-8 space-y-8 shadow-sm">
        ...
        <div className="flex items-center gap-4 pb-8 border-b border-border">
          ...
          />
          <div>
            <h2 className="text-xl font-semibold mb-1">{selectedBarber?.name || "Loading..."}</h2>
            <p className="text-muted-foreground">{selectedBarber?.experience_years} yrs exp</p>
          </div>
        ...
        <div className="space-y-5 pb-8 border-b border-border">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-lg">{selectedService?.name || "Loading..."}</h3>
                <Link to="/book/service" className="text-sm font-medium text-primary hover:underline">Edit</Link>
              </div>
              <p className="text-muted-foreground mt-1">{selectedService?.duration_minutes}m duration</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-lg">{date || "No date selected"}</h3>
